  ...
  { name: "Attorneys", href: "/attorneys/osbelia-castillo" },
  {
    ...
    dropdownItems: [
      ...
      },
      {
        ...
        href: "/practice-areas/personal-injury/sexual-molestation-abuse",
      },
    ],
  },
  {
    ...
    dropdownItems: [
      ...
      },
      {
        ...
        href: "/practice-areas/vehicle-accidents/crashes-due-to-cellphone-use",
      },
      {
...
const Navbar = () => {
  ...
  return (
    <>
      {/* 1. MOBILE OVERLAY BUTTON (Higher than everything else) */}
      <div className="xl:hidden fixed top-5 right-5 z-[110] flex items-center gap-4">
        <button
          onClick={() => setIsMenuOpen(!isMenuOpen)}
          className="p-2 text-gray-900 bg-white/80 backdrop-blur-md rounded-lg shadow-sm"
      ...
      <nav className="fixed top-0 left-0 w-full bg-white border-b border-gray-100 z-50 shadow-sm">
        <div className="max-w-[1600px] mx-auto flex items-center h-20 px-4 sm:px-6">
            ...
            {navLinks.map((link) => (
              ...
              >
                ...
                >
                  ...
                  {link.dropdownItems && (
                    <ChevronDown
                      className={`w-3.5 h-3.5 transition-transform duration-300 ${activeTab === link.name ? "rotate-180 text-[#C5A059]" : "opacity-50"}`}
                    />
                  )}
                </Link>
          ...
          <div className="hidden xl:flex flex-none pl-6 border-l border-gray-100">
            <Link
              href="tel:8052837656"
              className="flex items-center gap-2 px-5 py-3 bg-[#0A1128] text-white text-[12px] font-bold tracking-widest uppercase rounded-full hover:bg-[#C5A059] transition-all transform hover:scale-105 active:scale-95"
            >
      ...
      <div
        ref={menuRef}
        className="fixed inset-0 bg-white z-[100] xl:hidden flex flex-col pt-24 px-6 sm:px-10 invisible overflow-y-auto"
            ...
            <div key={link.name} className="py-1">
              <div className="flex items-center justify-between">
                ...
                {link.dropdownItems && (
                  ...
                  >
                    <ChevronDown
                      className={`w-5 h-5 transition-transform duration-300 ${mobileDropdown === link.name ? "rotate-180 text-[#C5A059]" : ""}`}
                    />
                  </button>
                )}
              </div>
              ...
              {link.dropdownItems && (
                <div
                  className={`overflow-hidden transition-all duration-300 ease-in-out ${mobileDropdown === link.name ? "max-h-[600px] opacity-100" : "max-h-0 opacity-0"}`}
                >
